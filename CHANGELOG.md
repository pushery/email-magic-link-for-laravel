...
- **The code boxes are centered by WireKit's own prop, and the WireKit floor for the screens moves to 2.54.** The layout centered the row of boxes with a `justify-content` rule written over `otp-input`'s internal `role="group"` markup, because the component took no prop for it; the 0.29.0 entry said the rule would go once WireKit offered one. WireKit 2.54 did, as `justify`, and the one-time-code screen now passes `justify="center"` while the layout reaches into the component nowhere. The suggestion and `require-dev` name 2.54 accordingly; on 2.53 the prop is ignored and the boxes sit at the left edge of the card.
- **An earlier release note claimed a framework limit that does not exist.** The entry that added the umbrella publish tag said `--tag` took one group name at a time and there was no way to publish everything. `vendor:publish --tag` accepts several tags, and `--provider` publishes every group of a provider; the `email-magic-link` tag is a convenience, one name instead of four that a later group joins by itself.
- **An earlier release note named the wrong tools and the wrong count for the purge's query log.** The entry that switched the log off said the command issues one statement per chunk and that the log is on "under Telescope, a debugbar, or any host that enables it". Each chunk is two statements, a locking select and a delete that binds every id of the chunk. Telescope and Debugbar record queries through the `QueryExecuted` event and never turn the log on, so they were never affected; a host that calls `enableQueryLog()` in a scheduled context was.
- **An earlier release note overstated what the purge indexes buy.** The 0.25.0 entry for the index migration said the purge plan drops "from a sequential scan at 35.8 ms to a bitmap scan over both indexes at 0.62 ms". The two readings came from differently warmed caches, and from a statement the purge no longer runs. Measured again on the chunk the purge sends today, a locking select ordered by id, with both sides warm: the index changes nothing while most rows qualify, makes a chunk about eight times faster when 1% qualify, and a steady-state purge about twenty-four times; the migration's docblock carries the table.
- **An earlier release note named a method that does not exist.** The entry about the raw address in a limiter key said the case is visible only on a host calling `RateLimiter::shouldHashKeys(false)`. Neither the `RateLimiter` class nor its facade has that method; the switch is `Illuminate\Routing\Middleware\ThrottleRequests::shouldHashKeys(false)`. A host checking whether it was affected should search for that call.
- **The Packagist page links the documentation and names the publisher.** `composer.json` carries `support.docs`, pointing at the portal where the documentation lives, and an `authors` entry; before, the page offered neither. The description now says what the Fortify handoff does, that a user with two-factor authentication reaches the challenge before any session exists, instead of calling it correct.
- **The scheduled purge carries a description and no separate name.** An earlier release note said the entry got "a name and a description"; for a scheduled command Laravel has one label, and `name()` sets the same description, so the second call won. The provider now sets the description alone. `schedule:list` shows what it showed before.
- **An expired or tampered sign-in link gets the configured refusal and fires `MagicLinkConsumptionFailed`.** The signature on a link expires together with its token, so the `signed` middleware answered the most common failure there is, an expired link, with Laravel's bare `403 Invalid signature.` before this package ran: no way back to a new link, no event for a host that counts failures, and a different answer from a spent link with a valid signature. Both link routes now check the signature themselves and fold a failure into the same refusal every dead token gets. The event reports `ClaimFailure::Expired` for an expired signature and `ClaimFailure::NotFound` for one that never held. A JSON client that posted a bare token used to receive the framework's `403` without an `error` key; it now receives the `422` with `"error": "invalid_or_expired"` every other dead token gets.
- **The WireKit screens no longer send their layout notes to the browser.** The explanations inside the inline `<style>` block are Blade comments now, so they stay in the view file and every page is a little smaller. The declarations are unchanged.
- **The installation page states the database floor.** Both purges claim rows with `FOR UPDATE SKIP LOCKED`, so the package needs PostgreSQL 10+ or MySQL 8.0.1+, while Laravel itself also installs on MySQL 5.7. It is tested on PostgreSQL 18 and MySQL 8.4.

### Fixed

- **The WireKit screens hand the CSP nonce to your own stylesheets too.** The layout gave the nonce to WireKit's tags, to Livewire and to its inline style, and not to the `ui.styles` links or the Vite tags that supply the utility classes the components render with. Under a nonce-based policy those were rejected and the screens rendered without your styles, with nothing logged. The `ui.styles` links now carry the nonce, and Vite is handed it when it has none of its own; a nonce you already set through `Vite::useCspNonce()` stands.
- **`email-magic-link:doctor` says what a missing CSP nonce breaks.** With no nonce source it warned that "the bundled inline script would be blocked". The resend countdown has been a same-origin file since 0.22.0 and needs a nonce only under a `strict-dynamic` policy; what a strict policy does block without one is the inline stylesheet both layouts carry, so the screens render unstyled. The line says that now, and the `ScriptNonce` contract and the `ui.script_nonce` comment in the config list the tags the nonce is for.
- **With the whole channel switched off, `invite()` names the switch that is off.** `InvitationsDisabledException` told the host to set `email-magic-link.invitations.enabled` to true, also when that key was already true and `email-magic-link.enabled` was the one set to false. It now says the channel is off and names that key. The exception class is unchanged, so a host catching it keeps working.
- **A mistyped `invitations.handler` says that the class does not exist.** The boot guard and the container binding answered a class name that resolves to nothing with the contract message, "[App\Handler] must implement [InvitationHandler]", which sends the reader looking for a missing interface on a class that is not there. Both now say the configured class does not exist and name the key; a class that exists and does not implement the contract still gets the contract message.
- **Under Octane, the package reads the configuration a request sets.** Octane swaps a copy of the config repository in for every request, and the package's configuration kept the instance it was built with at boot: a value set during a request never reached it, and with no `guard` configured, `issueLink()` used the boot-time default guard rather than the one `Auth::shouldUse()` had switched to. It now reads the repository in force.
- **`email-magic-link:purge` keeps no query log on an invitation model with a connection of its own.** The command switches the query log off while it runs, because it issues two statements per chunk. It did so only on the token model's connection, so a host that maps the invitation model onto another connection kept logging every invitation chunk. Both connections are switched off for the run and restored afterwards.
- **An expiry past January 2038 works on MySQL and MariaDB.** `expires_at` was a MySQL `TIMESTAMP`, which ends at 2038-01-19 03:14:07 UTC: an invitation or a token expiring later failed to be issued under strict mode, and without it was stored as a zero date and expired at once. A new migration makes both columns `DATETIME`, which reaches the year 9999. Run `php artisan migrate` after upgrading. PostgreSQL and SQLite are not affected, and on them the migration does nothing.
- **An hourly purge that was killed no longer skips the rest of the day.** The scheduled purge takes a lock so a long run cannot stack, and a process killed by a memory or time limit never releases it. The lock held for a day, so an hourly purge skipped the next twenty-three runs without an error or a log line. It now lapses within one interval: 55 minutes for an hourly purge, 23 hours for a daily one, six days for a weekly or monthly one.
- **`email-magic-link:install` no longer reports a config it did not write.** `vendor:publish` keeps an existing `config/email-magic-link.php` unless it is forced, and the command still printed "Published". It now says the file was kept and points at `--force` and at `email-magic-link:doctor`, which lists the keys the published file is missing.
- **The sign-in mail says how often a multi-use link works, and the confirmation page no longer calls it single-use.** Both said "can be used once" whatever `max_uses` or `issueLink(maxUses: n)` allowed, and that is the risky direction: whoever reads it treats the mail as spent after signing in and forwards it, while it still opens the account until it expires. The mail now reads "can be used 3 times" for such a link, in all bundled languages, and the confirmation page drops the sentence. `MagicLinkNotification` takes the number of uses as a new fifth constructor argument, which defaults to 1.
- **A two-factor account can sign in by magic link from an SPA with Fortify's views off.** Fortify registers its GET challenge page only when its views are on, and its documentation tells single-page applications to turn them off. The JSON answer was built from the browser redirect to that page, so on exactly the setup the JSON contract is for, the sign-in failed with a 500 after the handoff had started, and every attempt spent a link. The JSON answer now resolves the challenge itself: the configured page when it exists, otherwise `two-factor.login.store`, the URI the code is posted to. When neither exists, `redirect` is left out, as in Fortify's own answer.
- **A host that migrates its own copy of the tables keeps the scheduled purge.** `ignoreMigrations()` is what a host calls when it renames its published migrations, and such a host has the tables. The provider read the same call as "the tables were declined" and dropped the purge that `prune.schedule` asked for, with no log and no doctor line, so the token table grew without bound. `ignoreMigrations(tablesExist: true)` now says the tables exist and keeps the entry; a bare `ignoreMigrations()` still leaves it out, because a nightly command against missing tables would fail every night. `email-magic-link:doctor` reports a schedule that was asked for and left out, and the install instructions name the argument.
- **`email-magic-link:purge` clears settled invitations after invitations were switched off again.** The migrations create the invitations table on every install, but the purge only touched it while `invitations.enabled` was true. A host that used invitations and turned them off kept every accepted, revoked and expired invitation, each with its address in the clear, for good, and `retain_accepted_days` held only while the feature was on. The purge now runs whenever the table exists. Its second output line still appears only when invitations are on or something was deleted.
- **An empty `EMAIL_MAGIC_LINK_TTL` or `EMAIL_MAGIC_LINK_INVITATION_TTL` falls back to the default instead of becoming 0.** The config file cast both with `(int)`, and `env()` returns a variable that is set but empty as `""`, which casts to 0. A line left empty from an `.env` template therefore gave links a lifetime of 0, and the entropy guard refused to boot on every request, naming a value nobody had set; for invitations it meant expiry after the one-minute floor. The config now passes the raw value on, and the package reads anything that is not a number as the default, 900 and 604800 seconds. A published config keeps the old cast until those two lines are copied over.
- **Rolling back the index migration by one step keeps the invitation indexes.** The migration that indexes the purge predicates skips the two invitation indexes, because the table's own migration creates them, but its `down()` dropped them anyway. After `migrate:rollback --step=1` the invitations table was left without the indexes its purge filters on, while its own migration still counted as run. `down()` now removes only the `consumed_at` index it created.
- **The two-factor handoff hands Fortify the primary key.** Fortify stores `login.id` as the model's primary key and its challenge reads it back with `find()`. The handoff wrote `getAuthIdentifier()` instead, the same value only while a user model names no other identifier column; a model identified by a UUID or another column beside its numeric key got a challenge that found no account, or a different one, and could not sign in by link.
- **`ui.html_class` drops a no-break or zero-width space around the class.** The value was trimmed of ASCII whitespace only, and HTML splits classes at ASCII whitespace only, so a `dark` copied in with a no-break space became a class that WireKit's `.dark` never matched, and the screens stayed light without a word.
- **The Portuguese invitation message keeps its catalog's form of address.** European Portuguese (`pt-PT`) addresses the reader informally everywhere else, and this one line used the formal form; the base `pt` catalog, also served for `pt-BR`, mixed an informal pronoun into a formal sentence. Both now read consistently, and `pt` asks whoever sent the invitation without a gendered pronoun.

### Security

- **A lookup a host binds in the container is gated like the configured one.** The sign-in eligibility gate and the address check wrapped only the lookup named in `user_lookup`. A host that followed "bind your own" and bound `UserLookup` in its own service provider replaced the binding whole, gate included, so an account the eligibility contract refused was still mailed a link, and the address check that keeps a look-alike from receiving one fell away with it. Both now wrap whatever the binding resolves; redemption refused such an account already.
- **Wrong passphrases spend a passphrase-gated link, the way wrong codes spend a code.** A passphrase guards a link that reached the wrong hands, and it is a secret a person chose, yet wrong passphrases were bounded only by the consume rate limiter, for the whole lifetime of the link. They are now counted, and after `max_attempts_per_token` wrong ones the link is spent, all its remaining uses included. The response stays the same generic refusal.
- **Wrong codes are counted through the configured token model.** The count of wrong codes is what spends a code after `max_attempts_per_token`, and it was the one statement of the token store that named its table by hand. A token model mapped in `email-magic-link.models` with a table of its own had every other query follow it there while the counter looked at `magic_link_tokens`: each wrong guess was a 500 when that table was gone, and when it was still there the count read as 0 and the lockout never came, leaving only the rate limiter to bound guessing. The counter now goes through the configured model, its table, its connection and its global scopes.
- **A lock store that cannot lock fails the same way for every address.** A code is issued under a lock, and the request endpoint takes it only for an address that resolves to an account. With `lock_store` on a store that cannot lock, the refusal therefore happened only for known addresses: a 500 against the redirect an unknown address gets, which told a caller which accounts exist. In code mode the store is now checked before the lookup, so the misconfiguration answers every address alike. Link mode takes no lock and is unaffected.
- **A queued event listener no longer tells a known address from an unknown one.** Every event carries the request, and a request cannot be serialized, so a host listener that implements `ShouldQueue`, the ordinary way to write an audit log, made the dispatch throw before any queue saw it. The request endpoint dispatches only for an address that resolves to an account, so a known address got a 500 and an unknown one the redirect, and sign-in was broken for every account on top of it. The events now serialize without the request, which a queued listener finds as `null`; the `request` property is typed `?Request` accordingly, and a synchronous listener still receives it. An exception thrown by a listener on that branch of the request endpoint is reported and no longer changes the response.
- **An empty `EMAIL_MAGIC_LINK_FORTIFY` or `EMAIL_MAGIC_LINK_RESEND` no longer switches a protection off.** `env()` returns a variable that is set but empty as `""`, and that read as false: a line left empty from an `.env` template turned off the two-factor handoff, so accounts with a confirmed second factor signed in by link alone, or turned off the resend flood guard. Both now read an empty or blank value as not set, and keep `'auto'` and `true`. A readable `false`, `0` or `off` still switches them off. A published config keeps the old `resend.enabled` line and its old reading; copy the new line from the package's config.
- **An invitation signs in only a user of its own guard.** A session keeps only the user's identifier and resolves it through the guard's own provider on every later request. When an invitation handler returned a model of a different provider, for example the documented example's `User` on a guard backed by an `Admin` model, the invited person became whichever account of that provider carried the same id. The acceptance now checks the returned user against the guard's provider, as the Mint API does before it issues, and a mismatch fails with an `InvitationsMisconfiguredException` that names the guard. The acceptance rolls back with it, so the invitation stays open until the handler is fixed.
- **Closing a guard closes its invitations, and they can still be revoked.** Removing a guard from `email-magic-link.guards` stopped links and codes at once, because redemption re-checks the list, but every open invitation on that guard kept signing its recipient in there for its whole lifetime, seven days by default. The acceptance screen now refuses such an invitation, and posting it withdraws the invitation instead of accepting it, reported through `InvitationRejected` as `ClaimFailure::Revoked`. `InvitationIssuer::revoke()` no longer refuses a guard that has left the list: revoking closes something and opens nothing, and it used to leave the operator no way to withdraw the invitations of the guard they had just closed. A guard nothing was issued on now revokes nothing instead of throwing `UnknownGuardException`.
- **The entropy guardrail checks the code settings in link mode too.** The mode decides what the request endpoint sends, but `issueCode()` and the code route exist in every mode, so a link-mode install that mailed codes by another channel could boot with a four-digit numeric code and five attempts, and even with a one-character alphabet and no attempt cap. The check now runs whatever the mode. An install that never changed the code settings is unaffected; one that set them below the guardrail in link mode now fails to boot with the message that names the key to fix.
- **A link or code now goes to the address the account states, and an account resolves only for its own address.** Until now the credential was mailed to the string that was typed, and that string is only as equal to the account's address as the database finds it. MySQL's default collations ignore accents, so `victim@exämple.com` found the account stored as `victim@example.com`, and the mailer delivered to `victim@xn--exmple-cua.com`: a different domain that anyone can register. Whoever owned that look-alike received a working link or code for the account, and the code screen, which looks the account up again by what is typed there, signed them in. Two changes close it. The mail goes to the account's own mail route, or its `email` attribute when it declares none, the way Laravel's password reset does. And every lookup, including one you bind yourself, now resolves an account only when its stored address, trimmed and lowercased, is exactly the typed address, trimmed and lowercased; anything else is answered like an unknown address. An account whose lookup returns no readable `email` attribute can no longer be checked and resolves to nothing.
- **The queued sign-in mail is encrypted on the queue.** A token is stored only as an HMAC so that somebody who can read the database cannot sign in with it, but the mail carrying the plaintext link or code was queued in the clear, and Laravel's default queue is a table in that same database. Until a worker picked it up, the credential sat readable in `jobs`, and a job that failed kept it in `failed_jobs` for good. `MagicLinkNotification` now implements `ShouldBeEncrypted`, so the payload is encrypted with the application key. A notification you configure has to extend it and inherits the marker. Jobs queued before the upgrade are unaffected and still deliver.
...
- **An account can be refused a session without the refusal becoming an oracle.** The new `SignInEligibility` contract answers one question — may this account sign in to this guard right now — and what suspended, dormant or deleted-in-grace mean stays the application's business. It is consulted at **both** ends of the flow, and the second one is the point: gating issuance alone leaves a window as wide as a link's lifetime, because a link handed out a minute before an account was suspended is still a valid credential afterwards and nothing looks the address up when it comes back. On issuance a refused account resolves to nothing, so the endpoint answers for it exactly as it answers for an address nobody ever registered, down to the response bytes — a distinguishable refusal would let a caller send one link and learn both that the account exists and that it is suspended. On redemption the token is spent either way, the fail-closed direction the guard-allowlist check beside it already takes, and the refusal is reported as the new `ClaimFailure::Ineligible` rather than folded into `NotFound`: the HTTP response stays generic, so the event is the only place a host can tell a suspended account holding a valid link from ordinary noise. The gate sits on the resolved `UserLookup` rather than in the two controllers that call it, so a host binding its own lookup is covered too and a third caller added later inherits it instead of forgetting it. Invitation acceptance is deliberately outside the gate: it runs through your own `InvitationHandler`, which is the thing deciding what the account becomes, and asking whether it may sign in during the request that puts it into service would be asking you to contradict yourself. **Nothing changes for an install that says nothing** — the default allows every account that exists, because the package cannot know which column on your user model means suspended, and a default that guessed would either find nothing and lull or find the wrong thing and lock people out of an application that never asked for the feature.

### Changed

- **Two packages the shipped code imports directly are now declared in `require`: `nesbot/carbon` and `symfony/http-foundation`.** They were reached only through `laravel/framework`'s own dependency tree, so nothing about resolution changes -- the lockfile holds 197 packages before and after, and no consumer installs anything new. What changes is who promises their version: an undeclared import is one Composer may resolve below what this code calls, and the failure then surfaces as a missing method in a class this manifest never names. Each floor is the **base of its major** -- `^3.0` and `^7.0 || ^8.0` -- rather than the tighter constraint `laravel/framework` happens to carry today, because a floor decides who may install this package and a later minor would shut out an application the code runs on perfectly well. `Carbon\CarbonInterface` and `Symfony\Component\HttpFoundation\Response` predate both bases by several majors.
- **The code boxes group themselves now, and the WireKit floor moves to 2.53.** The one-time-code screen used to lay its boxes out with a grid written over WireKit's own `role="group"` markup, because `otp-input` wrapped wherever the width ran out: on the card an eight-character code broke 6+2 on a desktop and 5+3 on a 375px phone, at no boundary the code has, which reads as a rendering fault to somebody copying it out of an email. WireKit 2.53 takes a `group` prop whose groups do not break inside, so a row that fits stays one row and one that does not breaks where the code itself has a boundary. The result is the same at every width that was measured — eight boxes are one row at 1280px and 4+4 at 375px and 320px, six are one row and 3+3 — and it now moves with the component instead of reaching over its markup. **Two things did not change and are worth naming.** The card still widens for a long code, because how wide this package's card may grow is a decision about the screen around the component rather than about the component; that width is half a rem larger than before, since WireKit separates groups with a wider gap than it separates boxes and the row is correspondingly wider. And one declaration still reaches in — `justify-content: center`, because the boxes would otherwise sit against the left edge of a card whose every other element is centered, and `otp-input` takes no prop for it while its attribute bag lands on the outer wrapper rather than on that row. Reported upstream; it goes when that lands.
